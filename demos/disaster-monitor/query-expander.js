...
var INCLUDE_INBOUND_LINKS = true;
var INCLUDE_OUTBOUND_LINKS = true;
var USER_AGENT =
...
var LANGUAGE_LINKS_URL = 'http://' + BASE_LANGUAGE +
    ...
    '&lllimit=max&titles=';
var REDIRECTS_URL = '.wikipedia.org/w/api.php?action=query&blnamespace=0' +
    '&list=backlinks&blfilterredir=redirects&bllimit=max&format=json&bltitle=';
var INBOUND_LINKS_URL = '.wikipedia.org/w/api.php' +
    '?action=query&list=backlinks&bllimit=max&blnamespace=0&format=json' +
    '&bltitle=';
var OUTBOUND_LINKS_URL = '.wikipedia.org/w/api.php' +
...
var CATEGORY_NAMESPACES = (function() {
  ...
  temp.forEach(function(c) {
    ...
    if (rightToLeft.indexOf(c.lang) === -1) {
      categories[comp[0]] = true;
    } else {
      categories[comp[1]] = true;
    }
  });
...
var queryExpander = {
  pending: 1,
  seen: {},
  expandQueries: function(init, mainCallback) {
    ...
    async.waterfall([
      function(waterFallCallback) {
        if (isCategoryPage(init)) {
          ...
          that.pending = 1;
          that.seen = {};
          that.getCategoryLinks(init, {}, waterFallCallback);
        } else {
          // ...First, scrape all "Main article:" links, i.e., disaster types
          that.getMainLinks(init, waterFallCallback);
        }
      },
          ...
          function(err, results) {
            ...
            for (var disaster in results) {
              ...
              var disasterArticle = results[disaster].canonical
                  .replace(/^.*?\/wiki\/(.*?)$/g, '$1');
              var disasterUrl = LANGUAGE_LINKS_URL + disasterArticle;
                ...
                functions[disaster] = function(innerCallback) {
                  ...
                  request.get(options, function(err, response, body) {
                    if (!err && response.statusCode === 200) {
                      ...
                      var langLinks = {};
                      langLinks[BASE_LANGUAGE] = {
                        // As on Wikipedia: "Blizzard_(météorologie)"
                        url: 'http://' + BASE_LANGUAGE +
                            '.wikipedia.org/wiki/' +
                      ...
                      that.getRedirects(langLinks, function(err,
                        ...
                        var options = {
                          url: WIKIDATA_URL + langLinks[BASE_LANGUAGE].title
                              .replace(/\s/g, '_'),
                          headers: HEADERS
                        };
    ...
    for (var lang in langLinks) {
      (function(innerLang) {
        functions[innerLang] = function(callback) {
          var options = {
            url: 'http://' + innerLang + REDIRECTS_URL +
  ...
  getMainLinks: function(init, callback) {
    var options = {
      url: WIKI_BASE_URL + init,
      headers: HEADERS
    };
  ...
  getCategoryLinks: function(init, links, callback) {
    var url;
    if (isCategoryPage(init)) {
      url = WIKI_BASE_URL + init;
    } else {
    ...
    request.get(options, function(err, response, body) {
      ...
      $(CSS_QUERIES.subCategoryLink).each(function(i, elem) {
        ...
        that.pending++;
        that.getCategoryLinks(current, links, callback);
      });
  ...
  getInboundLinks: function(title, language, callback) {
    var options = {
      url: 'http://' + language + INBOUND_LINKS_URL + title,
      headers: HEADERS
    };
  ...
  getOutboundLinks: function(title, language, callback) {
    var options = {
      url: 'http://' + language + OUTBOUND_LINKS_URL + title,
      headers: HEADERS
    };
