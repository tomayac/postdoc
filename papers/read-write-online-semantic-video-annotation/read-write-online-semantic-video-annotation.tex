...
that are not displayed (by default) by the user agent.
...
\todo{PA: Might be a little too technical for a scientific paper,
...
\todo{PA: strange to mention this in a section dedicated to Media Fragments.
I understand that it is not worth having a whole section for it,
but still, this is a different REC,
so the current section-title is misleading.}
