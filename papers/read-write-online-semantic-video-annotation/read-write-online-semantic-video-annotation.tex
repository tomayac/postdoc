...
\maketitle
\begin{abstract}
Propose a~technology stack based on standard Web technologies
that allows for fully Read/Write-enabled semantic video annotation
that maximally leverages native functionality
already present in modern Web browsers.
%TS: More to come here.
\end{abstract}
...
namely \emph{(i)}~\emph{interactivity}, which, for example,
