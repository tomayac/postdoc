...
\begin{description}[leftmargin=*]
  \item[Style:] Representational annotations of
    type \texttt{style}
    allow for adding Cascading Style Sheet%
    \footnote{\url{http://www.w3.org/Style/CSS/}}
    styles to the corresponding \texttt{video} element
    of the current \texttt{TextTrack} element.
    This allows for the full range of visual effects,
  ...
  \item[Playback Rate:] The \texttt{playbackRate}
    ...
    playback rate of the corresponding \texttt{video} element
    of the current \texttt{TextTrack} element.
    The playback rate is expressed as a~multiple or fraction
    of the intrinsic speed of the video.
  \item[Volume:] The \texttt{volume} annotation type allows for
    specifying the volume of the corresponding \texttt{video}
    element of the current \texttt{TextTrack} element
    on a~scale from 0.0 to 1.0.
  \item[HTML:] Via the \texttt{html} annotation type,
    freeform HTML code can be added on top of the corresponding
    \texttt{video} element of the current \texttt{TextTrack}
    element. This allows for the addition of graphical,
    textual, or combined overlays that can also contain
    hyperlinks to other videos or media fragments within the
    current video.
\end{description}
