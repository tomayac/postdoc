...
using an Elastic Compute Cloud job that we have made available as open-source.%
\footnote{EC2 job:
...
we have matched each document for the regular expression
\texttt{<video[\^{}>]*>(.*?)</video>}, which proved very efficient.
We successfully tested exactly 2,247,615,323 webpages
that returned a successful HTTP response
and had to skip exactly non-HTML 46,524,336 documents.
On these webpages, we detected exactly 2,963,766 \texttt{<video>} tags,
resulting in a~1.37GB size raw text file that we have made available publicly.%
...
Looking at these 1,456 \texttt{<track>} nodes, we found a distribution of 
\begin{itemize}
  \item[kind=``captions'']: 915
  \item[kind=``subtitles'']: 525,
  \item[kind=``undefined'']: 10,
  \item[kind=``chapters'']: 2
\end{itemize}  

\begin{itemize}
  \item[srclang=``en'']: 1242,
  \item[srclang=``fr'']: 117,
  \item[srclang=``undefined'']: 78,
  \item[srclang=``en-US'']: 3,
  \item[srclang=``de'']: 8,
  \item[srclang=``th'']: 2,
  \item[srclang=``Español']: 1,
  \item[srclang=``Korean]: 1,
\end{itemize}  
\begin{comment}
labels
{ English: 1064,
  ...
  '"English"': 3 }
defaults
{ default: 650, undefined: 279, '': 526, true: 1 }
vtts
{ other: 1000, vtt: 66, srt: 390 }
numTracksPerVideo
{ '0': 9, '1': 1446, '9': 1 }
numSourcesPerVideo
{ '0': 49, '1': 826, '2': 173, '3': 404, '4': 4 }
types
{ 'video/mp4': 1279,
  undefined: 58,
  'video/webm': 94,
