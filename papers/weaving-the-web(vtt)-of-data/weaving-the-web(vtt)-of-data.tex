...
where the subject is any temporal fragment of the video,
identified by the corresponding Media Fragment URI.
They are encoded as \texttt{TextTrackCue}s
with \JSONLD\ payloads such as the one shown in \autoref{listing:webvtt}.
A dedicated context defines their semantics.
...
Our \JSONLD\ context eases common annotation tasks as
by defining the semantics of a few useful JSON properties
described below.
According to this context, \autoref{listing:webvtt} is interpreted as
\autoref{listing:jsonld} (JSON-LD) and \autoref{listing:rdftriples} (RDF).
Advanced tasks can of course be also supported by extending that context.\\
